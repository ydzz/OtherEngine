...
use crate::graphics::shader::{Shader,compile_glsl_shader};
...
impl<B> ShaderStore<B> where B:gfx_hal::Backend {
  ...
  let desc_set_layout = unsafe { 
                            self.device.borrow().create_descriptor_set_layout(&[],&[])
                                           .expect("Can't create descriptor set layout") 
                        };
  let desc_pool = unsafe {
                        self.device.borrow().create_descriptor_pool(0,&[],pso::DescriptorPoolCreateFlags::empty())
                                       .expect("Can't create descriptor pool") 
                      };
  let pipeline_layout = unsafe { self.device.borrow().create_pipeline_layout(
                                                   std::iter::once(&desc_set_layout),
                                                   &[(pso::ShaderStageFlags::VERTEX, 0 .. 8)]).unwrap()
                               };
  ...
  let vs_entry:pso::EntryPoint<B> = pso::EntryPoint { entry:"main",module:&vs_module, specialization: pso::Specialization::default() };
  let fs_entry:pso::EntryPoint<B> = pso::EntryPoint { entry:"main",module:&fs_module, specialization: pso::Specialization::default()};
  let shader_entries = pso::GraphicsShaderSet { vertex: vs_entry, hull: None,  domain: None, geometry: None,fragment: Some(fs_entry) };
  ...
  let mut pipeline_desc = pso::GraphicsPipelineDesc::new(
                    ...
                    pso::Rasterizer::FILL,
                    &pipeline_layout,
                    pass::Subpass { index: 0,main_pass:ref_pass});
  ...
  let raw_pipeline = unsafe { self.device.borrow().create_graphics_pipeline(&pipeline_desc, None).unwrap() };
  
  unsafe { self.device.borrow().destroy_shader_module(vs_module); }
  unsafe { self.device.borrow().destroy_shader_module(fs_module); }
